...
## Last Session
- **April 18, 2026**
- Overhauled **Task Detail Panel** with a tabbed interface (Description, Subtasks, Comments, Activity).
- Implemented functional **Subtasks** with progress tracking and LexoRank float positioning.
- Developed **Task Comments** system with real-time updates and optimistic UI.
- Built **List View** with grouping (status, priority, assignee), sorting, and density controls.
- Created **My Work** page aggregating user tasks with "Today" and "Upcoming" filters.
- Implemented **In-app Notification Centre** integrated into the Topbar.
- Resolved navigation 404s by providing placeholder pages for Inbox, Reporting, and People.
- Refactored core queries to support **Optimistic Updates** across all major mutations.
...
- [x] Drag-and-drop to reorder within a column
- [x] Configurable columns (add, rename, reorder, delete, set colour)
- [x] WIP limits per column (optional — turns column red when exceeded)
...
- [x] Assign to one or multiple members
- [x] Set due date and start date
- [x] Set priority (Urgent / High / Medium / Low / None)
- [x] Set status (per-project configurable statuses)
- [x] Add labels / tags
...
- [x] Create project (name, description, icon, colour, type, privacy)
- [x] Project templates (create from scratch or use preset template)
...
- [x] Create and manage sections/columns within project
...
- [x] Global search (tasks, projects, docs, members, comments)
- [ ] Filter search by type, project, date, assignee
- [ ] Search history / recent items
- [x] Keyboard shortcut to open search (Cmd/Ctrl + K)
- [x] Deep-link URLs for every task, project, doc, and view
- [ ] Recently visited pages section in sidebar
- [ ] Pinned/starred projects in sidebar
- [x] Quick task creation from anywhere (keyboard shortcut)
- [x] Command palette (keyboard-first power user navigation)
...
- [x] Project scaffold (Vite + React + TS + Tailwind + shadcn)
- [x] Supabase schema: orgs, workspaces, projects, members, roles
- [x] Auth flow (signup, login, invite, org creation)
- [x] Shell layout: sidebar (projects list, nav), topbar, command palette
- [x] RBAC + RLS policies
- [x] Design system tokens: colours, typography, spacing, shadows
- [x] Base component library: Button, Input, Modal, Dropdown, Avatar, Badge, Tooltip

### Phase 1 — Core Task Engine *(Weeks 4–8)*
- [x] Create, edit, delete, archive tasks
- [x] Task detail panel (slide-in or full page)
- [x] Subtasks (one level deep)
- [x] Assignees, due date, priority, status, labels
- [x] Sections within project
- [ ] Project-level custom statuses
- [x] Task comments (basic, no threading yet)
    > Completed April 2026 — Basic comment feed with optimistic updates.
- [ ] File attachments on tasks
- [x] Activity log on tasks
    > Completed April 2026 — Visual history feed in task panel.
- [x] **Kanban view** (full implementation — drag, columns, filters)
- [x] **List view** (full implementation — sort, group, inline edit)
    > Completed April 2026 — Multi-grouping support and inline editing.
- [x] My Work page (assigned to me, due today/this week)
    > Completed April 2026 — Centralized task view with date filtering.
- [x] Global search (basic)
- [x] In-app notification centre
    > Completed April 2026 — Real-time notification hub with 'Mark as Read'.
...
- [x] **Task panel vs full page** — Does clicking a task open a side panel (Asana-style) or navigate to a full task page (Linear-style)? Both have trade-offs. Lean toward side panel with option to expand.
